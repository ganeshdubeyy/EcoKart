...
const CheckoutPage = () => {
  ...
  const handleOnlinePayment = async () => {
    try {
      const loadingToastId = toast.loading("Redirecting to payment...")
      ...
      if (!stripe) {
        toast.dismiss(loadingToastId)
        return toast.error("Failed to initialize payment gateway")
      }

      // ✅ only send minimal fields
      const cartData = cartItemsList.map(item => ({
        _id: item._id,
        name: item.productId?.name || item.name, // safe fallback
      ...
      toast.dismiss()
      if (error) {
        return toast.error(error.message || "Unable to open payment page")
      }
